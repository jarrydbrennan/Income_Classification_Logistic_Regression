...
imbal_check = df[df['income'] == '>50K']['income'].value_counts().sum()/((df['income'].value_counts().sum()))*100
print('Income, the outcome variable, is unbalanced with',imbal_check.round(2),'% in the >50K class')
...
print(f'Accuracy Score on test set: {accuracy_score(y_test,y_pred)} via accuracy_score()')
# print(f'Accuracy Score on test set: {log_reg.score(x_test, y_test)} via log_reg.score()')
...
plt.figure(figsize = (10,7))
sns.barplot(coef_df['var'], coef_df['coef'])
plt.xlabel('Model Features')
plt.ylabel('Model Coefficients')
plt.title('Logistic Regression Coefficients Values')
